export interface WorkbenchTabRecord<Window extends object> {
  id: string;
  groupId: string;
  window: Window;
...
export class WorkbenchTabRegistry<Window extends object> {
  ...
  add(window: Window, id: string, groupId: string): WorkbenchTabRecord<Window> {
    const record = { id, groupId, window, sessionIds: new Set<string>(), activeCwd: undefined };
    this.#byWindow.set(window, record);
    return record;
  }

  get(window: Window): WorkbenchTabRecord<Window> | undefined {
    return this.#byWindow.get(window);
  }

  delete(window: Window): WorkbenchTabRecord<Window> | undefined {
    const record = this.#byWindow.get(window);
    this.#byWindow.delete(window);
    return record;
  }

  records(): WorkbenchTabRecord<Window>[] {
    return [...this.#byWindow.values()];
  }

  group(groupId: string): WorkbenchTabRecord<Window>[] {
  ...
  tabAt(window: Window, oneBasedIndex: number): WorkbenchTabRecord<Window> | undefined {
